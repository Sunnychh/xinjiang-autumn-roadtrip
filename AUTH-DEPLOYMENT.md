...
初始化命令通过终端隐藏输入密码；同名账号已存在时会失败，不重置原密码。开发库已初始化时可直接运行 `npm run dev`。
...
`npm test` 使用 Node 内置 SQLite 执行真实 SQL，验证认证、权限、会话撤销、请求限制与保护路径。测试仅使用独立临时数据库和测试凭据；不读取或修改你的账号。`.worker-assets/` 使用资源白名单，数据库、环境变量、后端源码与测试数据不会作为 Worker 静态资源发布。

Cloudflare 中每日清理到期会话和限流记录。D1 备份与恢复使用平台功能；不要把真实数据库导出提交 Git。当前版本仅管理账号，尚未接入相册、照片上传或攻略在线编辑。
